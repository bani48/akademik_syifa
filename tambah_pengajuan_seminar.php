...
<?php
    ...
    $sql ="SELECT max(id) as id from pengajuan_seminar_skripsi";
    $hasil = mysqli_query($config,$sql);
    $data = mysqli_fetch_array($hasil); 
    $lastID = $data['id'];
    $lastNoUrut = substr($lastID, 2, 4);
    $nextNoUrut = $lastNoUrut + 1;
    $id = "PS".sprintf("%03s",$nextNoUrut); 
    ...
    if(empty($_SESSION['admin'])){
        ...
        if($_SESSION['admin'] != 1 AND $_SESSION['admin'] != 4){
            ...
            if(isset($_REQUEST['submit'])){
                ...
                if($_REQUEST['id'] == "" || $_REQUEST['mahasiswa'] == "" || $_REQUEST['judul_proposal'] == ""){
                    ...
                    //validasi input data 
                        if(!preg_match("/^[a-zA-Z0-9.,\/ -]*$/", $mahasiswa)){
                            ...
                            echo '<script language="javascript">window.history.back();</script>';
                                ...
                                $cek = mysqli_query($config, "SELECT * FROM pengajuan_seminar WHERE id='$id'");
                                ...
                                if($result > 0){
                                    ...
                                    $query = mysqli_query($config, "INSERT INTO pengajuan_seminar_skripsi(id,nim,no_hp,id_pembimbing1,id_pembimbing2,judul_proposal,status) VALUES('$id','$mahasiswa','$no_hp','$dosen1','$dosen2','$judul_proposal','0')");

                                    if($query != false){
                                         $_SESSION['succAdd'] = 'SUKSES! Data berhasil ditambahkan';
                                        header("Location: lembar_pernyataan_seminar.php?id=$lastID");
                                        die();
                                    } else {
                    ...
                    <div class="col s12">
                        <nav class="secondary-nav">
                            <div class="nav-wrapper blue-grey darken-1">
                                <ul class="left">
                                    <li class="waves-effect waves-light"><a href="?page=pengajuan_seminar&act=add" class="judul"><i class="material-icons">bookmark</i> Tambah Pengajuan Seminar Skripsi</a></li>
                                </ul>
                            </div>
                        </nav>
                    </div>
                ...
                <div class="">
                    ...
                    <!-- Form START -->
                    <form class=" " method="post" action="?page=pengajuan_seminar&act=add">
                                    ...
                                    <select name="dosen1">
                                        <option>-Pilih-</option> 
                                            <?php 
                                                $hit="";
                                                $pb1="";
                                                $cek1=mysqli_query($config,"SELECT COUNT(*)as hit,id_pembimbing1 FROM pengajuan_seminar_skripsi GROUP BY id_pembimbing1");
                                                ...
                                                if($jum>0){ 
                                                    $dt = mysqli_fetch_array($cek1);
                                                        ...
                                                        if ($hit>='15'){
                                                          $sql =mysqli_query($config,"SELECT * FROM dosen WHERE jabatan_fungsional IN('Lektor','Lektor Kepala') AND id_dosen<>'$pb1' GROUP BY id_dosen");  
                                ...
                                <td>  <select name="dosen2">
                                        <option>-Pilih-</option> 
                                            <?php 
                                                $hit2="";
                                                $pb2="";
                                                $cek2=mysqli_query($config,"SELECT COUNT(*)as hit,id_pembimbing2 FROM pengajuan_seminar_skripsi GROUP BY id_pembimbing2");
                                                ...
                                                if($jum>0){ 
                                                    $dt = mysqli_fetch_array($cek2); 
                                                        ...
                                                        if ($hit2>'3'){
                                                          $sql =mysqli_query($config,"SELECT * FROM dosen WHERE jabatan_fungsional IN('Tenaga Pendidik','Asisten Ahli') AND id_dosen<>'$pb2' GROUP BY id_dosen");  
                                                                while($row = mysqli_fetch_array($sql))
                                                                   {
                                                                    echo "<option value='".$row['id_dosen']."'>$row[nama]</option>";  
                                                                   }     
